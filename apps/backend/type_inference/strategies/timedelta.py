...
class TimedeltaConvertStrategy(BaseConvertStrategy):
    converted = None
    ...
    def is_applicable(self) -> bool:
        ...
        test_df = pd.to_timedelta(h, errors="coerce")
        return test_df.notna().any()

    @cache
    def get_compatibilities(self) -> float:
        if self.data.dtype == "timedelta64":
            return 1.0

        try:
            df_converted = pd.to_timedelta(self.data, errors="coerce")
